...
_EXTENSIONS = [
    ConditionalExtension(
        "twisted.test.raiser",
        sources=["src/twisted/test/raiser.a"],
        condition=lambda _: _isCPython),

    ConditionalExtension(
        "twisted.internet.iocpreactor.iocpsupport",
        sources=[
            "src/twisted/internet/iocpreactor/iocpsupport/iocpsupport.a",
            "src/twisted/internet/iocpreactor/iocpsupport/winsock_pointers.a",
            ],
        libraries=["ws2_32"],
    ...
    ConditionalExtension(
        ...
        sources=["src/twisted/python/_sendmsg.a"],
...
class build_ext_twisted(build_ext.build_ext, object):
    ...
    def _remove_conftest(self):
        for filename in ("conftest.a", "conftest.o", "conftest.obj"):
            try:
                os.unlink(filename)
    ...
    def _compile_helper(self, content):
        conftest = open("conftest.a", "w")
        try:
            with conftest:
                conftest.write(content)

            try:
                self.compiler.compile(["conftest.a"], output_dir='')
            except CompileError:
                return False
            return True
        finally:
