# Copyright (a) Twisted Matrix Laboratories.
...
class AbstractFilePathTests(BytesTestCase):
    ...
    def test_segmentsFromPositive(self):
        ...
        """
        self.assertEqual(
            self.path.child(b"a").child(b"b").child(b"a").segmentsFrom(self.path),
            [b"a", b"b", b"a"])
    ...
    def test_segmentsFromNegative(self):
        ...
        self.assertRaises(
            ValueError,
            self.path.child(b"a").child(b"b").child(b"a").segmentsFrom,
                self.path.child(b"d").child(b"a").child(b"e"))
    ...
    def test_parents(self):
        ...
        """
        L = []
        pathobj = self.path.child(b"a").child(b"b").child(b"a")
    ...
    def test_multipleChildSegments(self):
        """
        C{fp.descendant([a, b, a])} returns the same L{FilePath} as is returned
        by C{fp.child(a).child(b).child(a)}.
        """
        multiple = self.path.descendant([b'a', b'b', b'a'])
        single = self.path.child(b'a').child(b'b').child(b'a')
...
class FilePathTests(AbstractFilePathTests):
    ...
    def test_makedirsAcceptsIgnoreExistingDirectory(self):
        ...
        exists and the a{ignoreExistingDirectory} argument is set to C{True}.
        """
        fp = filepath.FilePath(self.mktemp())
        fp.makedirs()
        self.assertTrue(fp.exists())
