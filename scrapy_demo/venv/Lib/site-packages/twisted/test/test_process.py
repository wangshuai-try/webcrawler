...
class ProcessTests(unittest.TestCase):
    ...
    def test_wrongArguments(self):
        ...
        for env in badEnvs:
            self.assertRaises(
                TypeError,
                reactor.spawnProcess, p, pyExe, [pyExe, b"-a", b""], env=env)
...
class PosixProcessBase(object):
    ...
    def test_abnormalTermination(self):
        ...
        p = TrivialProcessProtocol(d)
        reactor.spawnProcess(p, pyExe,
                             [pyExe, b'-a', b'import sys; sys.exit(1)'],
...
class PosixProcessTests(unittest.TestCase, PosixProcessBase):
    ...
    def test_process(self):
        ...
        p = Accumulator()
        d = p.endedDeferred = defer.Deferred()
        reactor.spawnProcess(p, cmd, [cmd, b"-a"], env=None, path="/tmp",
                             usePTY=self.usePTY)
        p.transport.write(s)
        p.transport.closeStdin()
...
class PosixProcessPTYTests(unittest.TestCase, PosixProcessBase):
    ...
    def test_badArgs(self):
        pyArgs = [pyExe, b"-u", b"-a", b"print('hello')"]
        p = Accumulator()
...
class DumbWin32ProcTests(unittest.TestCase):
    ...
    def test_pid(self):
        ...
        comspec = u"cmd.exe"
        cmd = [comspec, u"/a", pyExe, scriptPath]
...
class Win32CreateProcessFlagsTests(unittest.TestCase):
    ...
    def test_flags(self):
        ...
        to test this interactively::
            ...
            PING_EXE = r"a:\windows\system32\ping.exe"
        ...
        cmd = [comspec, "/a", exe, scriptPath.path]
