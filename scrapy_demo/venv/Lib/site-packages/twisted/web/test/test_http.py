...
class HTTP1_0Tests(unittest.TestCase, ResponseTestMixin):
    ...
    def test_transportNotAbortedWithZeroAbortTimeout(self):
        """
        If the L{HTTPChannel} has its a{abortTimeout} set to L{None}, it never
        aborts.
        """
        clock = Clock()
        transport = StringTransport()
...
class ChunkedTransferEncodingTests(unittest.TestCase):
    ...
    def test_short(self):
        ...
        for s in iterbytes(b'3\r\nabc\r\n5\r\n12345\r\n0\r\n\r\n'):
            p.dataReceived(s)
        self.assertEqual(L, [b'a', b'b', b'a', b'1', b'2', b'3', b'4', b'5'])
        self.assertEqual(finished, [b''])
...
class QueryArgumentsTests(unittest.TestCase):
    def testParseqs(self):
        self.assertEqual(
            cgi.parse_qs(b"a=b&d=a;+=f"),
            http.parse_qs(b"a=b&d=a;+=f"))
        self.assertRaises(
            ValueError, http.parse_qs, b"blah", strict_parsing=True)
        self.assertEqual(
            cgi.parse_qs(b"a=&b=a", keep_blank_values=1),
            http.parse_qs(b"a=&b=a", keep_blank_values=1))
        self.assertEqual(
            cgi.parse_qs(b"a=&b=a"),
            http.parse_qs(b"a=&b=a"))
